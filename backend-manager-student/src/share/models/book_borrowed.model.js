...
module.exports = {
    ...
    createBorrowBook: (data) => new Promise((resolve, reject) => {
        try {
            const result = knex('borrowed_book').insert(data).onConflict('borrowed_book_id').merge()
     ...
     */
    getBorrowBook: async (borrow_book_id) => {
        const result = await knex('borrowed_book')
            ...
            .select(
                'books.name', 'books.image_uri', 'books.description', 'books.page_number', 'borrowed_book.*', 'authors.name', 'authors.avatar_uri',
            )
